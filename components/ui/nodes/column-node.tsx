...
const ColumnNode: React.FC<ColumnNodeProps> = ({ data }) => {
  return (
    <div className="bg-amber-50 w-full h-full group relative border border-amber-200 rounded shadow-lg">
      <Handle type="target" position={Position.Left} className="invisible" />
      <div className="text-start mb-2 text-amber-900">{data.label}</div>
      {data.detail && (
